...
export const RequestSection = () => {
    ...
    const [selectedService, setSelectedService] = useState<string>("") // Inicia vazio para ser controlado

    const [services, setServices] = useState([
        { id: 1, name: "Plumbing", urgent: false, icon: <Hammer className="text-[#1E73EA] w-5 h-5" />, bg: "bg-blue-100" },
        { id: 2, name: "Electrical", urgent: false, icon: <Zap className="text-[#D89B00] w-5 h-5" />, bg: "bg-yellow-100" },
        { id: 3, name: "Carpentry", urgent: false, icon: <Paintbrush className="text-[#E28B00] w-5 h-5" />, bg: "bg-orange-100" },
        { id: 4, name: "Cleaning", urgent: false, icon: <BrushCleaning className="text-[#00A99D] w-5 h-5" />, bg: "bg-cyan-100" }
    ])

    const toggleUrgent = (id: number) => {
        setServices(prev =>
            prev.map(service =>
                service.id === id ? { ...service, urgent: !service.urgent } : service
            )
        )
    }
    ...
    const handlerequest = async () => {
        try {
            ...
            const orcamentoResponse = await fetch("http://localhost:8080/orcamento/create", {
                method: "POST",
                headers: {
            ...
            // CORREÇÃO: Verificação de sucesso (200 ou 201)
            ...
            const response = await fetch("http://localhost:8080/prestacao-servico/create", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${cookies.token}`
            ...
            if (response.ok) {
                ...
                setDescription("")
                setSelectedService("")
                setServices(prev => prev.map(s => ({ ...s, urgent: false })))
            } else {
    ...
    return (
        <main className="max-w-5xl mx-auto py-16 px-5">
            <div className="flex flex-col gap-3 mb-10">
                <h1 className="text-5xl font-bold text-[#0E1525]">Public Service Request</h1>
                <p className="text-xl text-[#6B7A99]">Select the service you need and submit your request.</p>
            </div>
            ...
            <Card className="rounded-2xl border border-gray-200 shadow-none overflow-hidden">
                <CardContent className="p-0">
                    ...
                    <RadioGroup value={selectedService} onValueChange={setSelectedService}>
                        {services.map((item) => (
                            <div key={item.id} className="grid grid-cols-3 items-center border-b border-gray-200 px-6 py-5">
                                <div className="flex items-center gap-3">
                                    <RadioGroupItem value={item.id.toString()} id={`service-${item.id}`} />
                                    <Label htmlFor={`service-${item.id}`} className="cursor-pointer">Select</Label>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-2xl text-[#0E1525]">{item.name}</span>
                                </div>
            ...
            <div className="mt-14 flex flex-col gap-5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-2xl font-bold text-[#0E1525]">Service Description & Notes</span>
                </div>
